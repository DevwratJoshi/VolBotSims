...
float lightWidth_initial = 20;
float lightWidth_max = 180;
float lightWidth_step = 20;

// The following is the distance of influence of the flashlight
float R_initial = big + mid*2; // The distance of influence is expressed in module lengths (ML).  The initial value is 2.5 ML. 
...
// End simulation initialization
 ...
 //////////// Destroy all robots if the record count exceeds the number of points we want (max_record)
 
 
      if(record % max_record == 0 && delay > DELAY)
      {
             steps = 0;
             record = 0;
             output.flush();
...
void setSimulationConditions()
{
   ...
   if(!endProgram)    
   output = createWriter("data_R_alpha_beta/" + R_fold + str(int(R))  + "/" + beta_fold + str(int(lightWidth)) + "/" + "alpha_fold"  + str(int(lightAngle)) + "/" + data_fold[in_counter-1] + "/" +  "Flash_flat_" + str(R)  +"_" + str(int(lightWidth))+ "_"+ str(int(lightAngle)));
