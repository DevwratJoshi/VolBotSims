  ...
  */
  ...
  ///////////////////////// Change robot size block
    ...
    for (int i = robots.size()-1; i >= 0; i--) 
    {
      ...
      {
        p.changeRadius(big);
        p.changeFriction(fric_high);
        if(p.colour != 'b' && p.colour != 'r')
        p.colour = 'y';
      } else if (p.decreasing)
      {
        p.changeRadius(small);
        p.changeFriction(fric_low);
      }
    }
...
void mousePressed()
{
 ...
 {
  Vec2 pos = r.checkPos();
  if(sqrt((mouseX-pos.x)*(mouseX-pos.x) + (mouseY-pos.y)*(mouseY-pos.y)) <= r.r)
  {
    if(r.colour == 'r')
    {
     r.colour = 'b'; 
