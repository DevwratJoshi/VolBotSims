...
void setup()
{
  ...
  output_data = createWriter(data_folder + "data.txt");
    ...
    for (int i = robots.size()-1; i >= 0; i--) {
      ...
      if(record > maxSteps || segregatorsTouching())
      {
       ...
       for (int i = robots.size()-1; i >= 0; i--)
       {
        ...
        if(p.type == 's')
        {
         output_data.print(((p.checkPos().x - box.checkPos().x + box_bottom/2)/(small*2)) + "," + ((box.checkPos().y - p.checkPos().y)/(small*2)) + ",");
         
        }
        p.killBody();
        robots.remove(i);
       }
       output_data.print(record +"\n");
   ...
   while ((line = reader.readLine()) != null) 
    {
      ...
      char colour = 'g';
         ...
         if(nums[3] == 1)
        {
          ...
          f = fric_high;
          t = 's';
          output_data.print(nums[0]/(small*2) + "," + nums[1]/(small*2) + ",");
